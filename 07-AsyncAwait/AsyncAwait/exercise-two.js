...
async function problemB() {
  ...
  // AsyncAwait version
  const promesas = filenames.map(f => promisifiedReadFile(f))
  // se crea un array de promesas [p1,p2,p3,p4...]

  const respuestaDePromesas = await Promise.all(promesas);
  // array de stanzas [stnaza1,stanza2,stanza3...]

  for (const res of respuestaDePromesas) {
    blue(res)
  }
  console.log('done')
}
