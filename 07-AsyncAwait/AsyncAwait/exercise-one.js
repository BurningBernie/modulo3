...
function problemB() {
  ...
  async function hola(filename) {
    blue(await promisifiedReadFile(filename))
  }

  hola('poem-one/stanza-02.txt')
  hola('poem-one/stanza-03.txt')
...
async function problemD() {
  ...
  try {
    const stanzax = await promisifiedReadFile('poem-one/wrong-file-name.txt')
    blue(stanzax)
    const stanza3 = await promisifiedReadFile('poem-one/stanza-03.txt')
    blue(stanza3)
  } catch (error) {
